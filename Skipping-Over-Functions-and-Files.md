...
> 如果你想进入函数`foo`和`bar`，但不想调试`boring`，那么如果你在第103行运行`step`，你将进入`boring()`，但如果你运行`next`，你将跳过`foo`和`boring`！
...
> 一个更灵活的解决方案是执行[跳过无聊]永远不会踏入“无聊”。现在，当您在第103行执行“步骤”时，您将跳过“无聊”并直接进入“foo”。
...
> 在POSIX系统中，正则表达式的格式为“扩展正则表达式”。例如，参见'man 7 regex' / Linux系统，了解glob样式模式的描述。
