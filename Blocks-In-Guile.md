---
tip: translate by openai@2023-06-23 18:01:18
...
---
...
> 在GDB中，符号存储在块中。块大致对应源代码中的一个作用域。块以层次结构组织，并以Guile中的<gdb:block>类型的对象单独表示。块依赖于可用的调试信息。
...
> 靠近全局块内部的块就是静态块。静态块通常用于保存文件作用域变量和函数。
...
> GDB提供了一种获取块的超块的方法，但目前没有办法检查块的子块，或遍历符号表中的所有块（参见[Guile中的符号表](Symbol-Tables-In-Guile.html#Symbol-Tables-In-Guile)）。
...
> 如果对象是一个<gdb:block>对象，则返回#t。否则返回#f。
...
> 返回`#t`如果`<gdb:block>`块`block`有效，则返回`#f`。如果指向的块在下级中不再存在，则块对象可能无效。在调用过程时，如果块无效，则其他所有`<gdb:block>`方法都会抛出异常。在迭代块的符号时也会检查块的有效性。
...
> 返回以<gdb:symbol>对象表示的<gdb:block>块的名称。如果块未命名，则返回#f。
...
> 如果父块不存在，则返回包含`<gdb：block>` `块`的块。如果父块不存在，则返回`#f`。
...
> 如果<gdb:block>块是一个全局块，则返回#t，否则返回#f。
...
> 返回`<gdb:block>`块中所有符号（作为\<gdb:symbol\>对象）的列表。
...
> 程序设计方案：**make-block-symbols-iterator** *block*
...
> 返回一个类型为`<gdb:iterator>`的对象，它将遍历块中的所有符号。Guile程序不应假定特定的块对象总是包含给定的符号，因为GDB功能和基础架构的变化可能会导致符号在符号表中移动到不同的块中。参见[Guile中的迭代器](Iterators-In-Guile.html#Iterators-In-Guile)。
...
> 返回#t，如果对象是<gdb：block-symbols-progress>对象。这个对象可以从`make-block-symbols-iterator`返回的`<gdb：iterator>`对象的`progress`元素中获得。
